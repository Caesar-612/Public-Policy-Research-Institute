...
race_table <- function(x){
  ...
  for (i in str_subset(colnames(x),"^Q")[1:5]){
    ...
    a1 <- crosstab(df= x, x = !!sym(i), y = race_m, weight = weight, pct_type = "column")
    a2 <- crosstab(df= x, x = !!sym(i), y= educ_m2, weight = weight, pct_type = "column")
    a3 <- crosstab(df= x, x = !!sym(i), y= region_m, weight = weight, pct_type = "column")
    a4 <- crosstab(df = x, x = !!sym(i), y = allcat_m, weight =weight, pct_type = "column" )%>%select("All Americans")%>%
    ...
    a3%>%
      ...
      rbind(c)%>%
      select(-i) ->r3
    a5 <- cbind(a1,a2,a3,a4)
    #change the row name to Unweight sample size
    a5[a5 == 1] <- "Unweighted sample size"
    a5%>%
...
religious_table <- function(x){
  for (i in str_subset(colnames(x),"^Q")[1:5]){
    b1 <- crosstab(df= x, x = !!sym(i), y = reltrad_w4, weight = weight, pct_type = "column")
    #select the column we need and save it back
    b1 <- b1[,1:13]
    #for r2 select(-i) will remove the column than we can avoid duplicate problem
    b2 <- crosstab(df = x, x = !!sym(i), y = allcat_m, weight =weight, pct_type = "column" )%>%select(-i)%>%
      rename("american" = "All Americans")
    ...
    b5 <- cbind(b1,b2)
    b5[b5 == 1] <- "Unweighted sample size"
    b5%>%
      relocate(american, .after = i)%>%
      kable(caption =get_label(x[i]), digits = 0,
            col.names =  c(" ", "All_Americans\nA", 'White\nevangelical\nProtestant\nB', 'White\nmainlineProtestant\nC', 'Black\nProtestant\nD', 'Hispanic\nProtestant\nE','Other\nnonwhite\nProtestant\nF', 'White\nCatholic\nG', 'Hispanic\nCatholic\nH', 'Mormon\nI', 'Other\nChristian\nJ', 'Jewish\nK', 'Other\nnon-Christian\nreligion\nL','Unaffiliated\nM')) %>%
      kable_styling("striped", full_width = T,
                    position = "left", font_size = 8) %>%
      add_header_above(c(" " = 1, "All Americans" = 1, "Religious Affiliation" = 12))-> b5
    print(b5)
  }
}
...
politics_table <- function(x){
  for (i in str_subset(colnames(x),"^Q")[1:5]){
    c1 <- crosstab(df= x, x = !!sym(i), y = party_m, weight = weight, pct_type = "column")
    #select the column we need and save it back
    c1 <- c1[,1:4]
    c2 <- crosstab(df = x, x = !!sym(i), y = republican_news, weight = weight, pct_type = "column")
    c2 <- c2[,1:5]
    c3 <- crosstab(df = x, x = !!sym(i), y = ideo_m, weight = weight, pct_type = "column")
    c3 <- c3[,1:4]
    c4 <- crosstab(df = x, x = !!sym(i), y = allcat_m, weight =weight, pct_type = "column" )%>%select(-i)%>%
      rename("american" = "All Americans")
    ...
    c3%>%
      ...
      rbind(c)%>%
      select(-i) ->c3
    c5 <- cbind(c1,c2,c3,c4)
    c5[c5 == 1] <- "Unweighted sample size"
...
vaccine_table <- function(x){
  for (i in str_subset(colnames(x),"^Q")[1:5]){
    d1 <- crosstab(df= x, x = !!sym(i), y = vaccine_m, weight = weight, pct_type = "column")
    d1 <- d1[,1:6]
    d2 <- crosstab(df = x, x = !!sym(i), y = hesitant_m, weight = weight, pct_type = "column")
    d2 <- d2[,1:4]
    d3 <- crosstab(df = x, x = !!sym(i), y = allcat_m, weight =weight, pct_type = "column" )%>%select(-i)%>%
      rename("american" = "All Americans")
    #another way to create unweighted sample size column
    x%>%
      group_by(vaccine_m)%>%
    ...
    d4 <- cbind(d1,d2,d3)
    d4[d4 == 1] <- "Unweighted sample size"
    d4%>%
      relocate(american, .after = i)%>%
      kable(caption =get_label(x[i]), digits = 0,
